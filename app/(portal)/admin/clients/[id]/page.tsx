...
import { ArrowLeft, Loader2, CreditCard, User, FileText, LifeBuoy, Package, MessageCircle } from 'lucide-react'
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <Tabs defaultValue="overview">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Card>
              <CardContent className="pt-4 flex items-center gap-3">
                ...
                <div><div className="text-2xl font-bold">{inventory.length}</div><div className="text-xs text-gray-500">Products</div></div>
              </CardContent>
            </Card>
          </div>
          {agreements.length > 0 && (
          ...
          {tickets.length > 0 ? (
            <div className="space-y-2">
              {tickets.map((t: Record<string, unknown>) => (
                <Card key={String(t.id)}><CardContent className="py-3 flex items-center justify-between">
                  <div><div className="text-sm font-medium">{String(t.subject)}</div><div className="text-xs text-gray-400">{new Date(String(t.created_at)).toLocaleDateString()}</div></div>
                  <div className="flex gap-2"><Badge variant="outline">{String(t.status)}</Badge><Badge variant="outline">{String(t.priority)}</Badge></div>
                </CardContent></Card>
              ))}
            </div>
        ...
        <TabsContent value="inventory" className="mt-4">
          {inventory.length > 0 ? (
            <div className="space-y-2">
              {inventory.map((p: Record<string, unknown>) => (
                <Card key={String(p.id)}><CardContent className="py-3 flex items-center justify-between">
                  <div><div className="text-sm font-medium">{String(p.product_name)}</div><div className="text-xs text-gray-400">{String(p.category || '')} {p.sku ? `\u00B7 ${p.sku}` : ''}</div></div>
