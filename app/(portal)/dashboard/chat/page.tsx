...
export default function ChatPage() {
  ...
  /**
   * Send a message. Accepts an optional `override` so quick-suggest buttons
   * can fire without waiting for React's async state update on `input`.
   */
  const sendMessage = useCallback(async (override?: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.suggestTicket) {
          setMessages((prev) => [
            ...prev,
            {
              role: 'assistant',
              content:
                "\n\n---\n\n\uD83C\uDF9F\uFE0F **Would you like me to create a support ticket for this?** I can escalate this to our team for a faster resolution.\n\nJust say \"create a ticket\" and I'll set it up for you.",
            },
          ])
        }
      } else {
        setMessages((prev) => [
          ...prev,
          { role: 'assistant', content: 'Sorry, I encountered an error. Please try again or contact support.' },
        ])
      }
    } catch {
      setMessages((prev) => [
  ...
  return (
    <div className="max-w-3xl mx-auto flex flex-col gap-4">
      ...
      {/* Card uses an explicit dvh-based height so it fills the viewport properly */}
      <Card
